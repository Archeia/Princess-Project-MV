 ...
 *
 * @plugindesc v1.2.0_Surps Support for RPGMaker maps alongside Tiled maps. Support for tileset tile properties.
 * @author Dr.Yami, Liquidize, and Surprising Sandwich (Michael Mitchell)
 * History:
 * v1.2.0 Implementation for Tiled Map Editor.
 */
...
(function() {
    ...
    Data.prototype.isReady = function() {
		if (!this.data || !this.tilesetsData || !this.tilesets) return false;
		if (this.tilesetsData.length != this.tilesets.length) return false;
		for (var i = 0; i < tilesets.length; i++) {
			if (!tilesets[i].isReady()) return false;
		}
		return true;
    };
...
(function() {
	...
	Tileset.prototype.getTile = function(id) {
		if (!this.tiles || !this.isInTileset(id)) return null;
		return this.tiles[(id - this.firstId).toString()];
	}
...
(function() {
    ...
    Layer.prototype._getTileset = function(tileId) {
        var i = 0,
			allTilesets = this.tilesets.tilesets;
		var 
            length = allTilesets.length,
            tileset; // for tilesets iterator

        for (; i < length; i++) {
            tileset = allTilesets[i];
    ...
    Layer.prototype._drawAnimationTile = function(tileId, dx, dy,lx,ly) {
		...
		var tiledata = [];
		if (tileset.tiles) {
			tiledata = tileset.getTile(tileId);
		}
        if (tiledata) {
...
(function() {
    ...
    Core.isMapLoaded = function() {
		if (!!Core.noMap) return true;
		if (!Core.dataMap || !Core.dataMap.tilesets) return false;
		/*for (var i = 0; i < Core.dataMap.tilesets.length; i++) {
			if (!Core.dataMap.tilesets[i].image) return false;
		}*/
		return true;
    };
    ...
    Core.prototype._setupTilesets = function() {
        ...
        for (; i < length; i++) {
			...
			if (data.image) {
				...
				var self = this;
				(function(tilesetIndex, oldData) {
					self._loadTilesetFromSource(oldData.source, function(newData) {
						tilesetsData[tilesetIndex] = newData;
						tilesetsData[tilesetIndex].originalData = oldData;
						tilesetsData[tilesetIndex].firstgid = oldData.firstgid;
						// "data" now has a .image
						self.tilesets.push(new Tileset(newData));
					});
				})(i, data);
			}
        }
	...
	Core.prototype._loadTilesetFromSource = function(source, callback) {
        var xhr = new XMLHttpRequest();
		var filepath = source;
        ...
        xhr.onreadystatechange = function() {
            if (xhr.readyState === 4) {
                if (xhr.status === 200 || xhr.responseText !== "") {
                    ...
                    throw new Error('[YED#Tilemap] Loading error on tileset: \'' + tilesetData.source + '\'');
                }
            }
        };
        xhr.send();
...
(function() {
    ...
    var _Game_Map_tilemapUpperLayers = Game_Map.prototype.tilemapUpperLayers;
	var _Game_Map_tilemapLowerLayers = Game_Map.prototype.tilemapLowerLayers;
	var _Game_Map_tilemapRefresh = Game_Map.prototype.tilemapRefresh;
	var _Game_Map_isPassable = Game_Map.prototype.isPassable;
	var _Game_Map_checkLayeredTilesAttribute = Game_Map.prototype.checkLayeredTilesAttribute;
    ...
    var _Game_Event_setupInitPosition = Game_Event.prototype.setupInitPosition;
    var _Spriteset_Map_createTilemap
        = Spriteset_Map.prototype.createTilemap;

	var _Tilemap_paintAllTiles = Tilemap.prototype._paintAllTiles;
	var _Tilemap_updateLayerPositions = Tilemap.prototype._updateLayerPositions;
	var _Tilemap_updateTransform = Tilemap.prototype.updateTransform;
	var _Tilemap_createLayers = Tilemap.prototype._createLayers;
	var _Tilemap_refresh = Tilemap.prototype.refresh;
    ...
    Game_Map.prototype.setup = function(mapId) {
		if (!YED.Tilemap.Core.noMap) this.setupYEDTilemap();
    ...
    Game_Map.prototype.tileWidth = function() {
		if (!YED.Tilemap.Core.noMap) 
			return this._yedTilemapData().tileWidth;
		else 
			return this.oldTileWidth();
    };
    ...
    Game_Map.prototype.tilemapUpperLayers = function() {
		if (!!YED.Tilemap.Core.noMap) return _Game_Map_tilemapUpperLayers;
		return YED.Tilemap.Core.singleton.upperLayers;
    };

    Game_Map.prototype.tilemapLowerLayers = function() {
		if (!!YED.Tilemap.Core.noMap) return _Game_Map_tilemapLowerLayers;
		return YED.Tilemap.Core.singleton.lowerLayers;
    };

    Game_Map.prototype.tilemapRefresh = function() {
		if (!!YED.Tilemap.Core.noMap) return _Game_Map_tilemapRefresh.call(this);
		return YED.Tilemap.Core.singleton.refresh();
    };

    Game_Map.prototype.isPassable = function(x, y, d) {
		if (!!YED.Tilemap.Core.noMap) return _Game_Map_isPassable.call(this, x, y, d);
        ...
        if (d === 4) {
            if (!(arrows[index] & 1) 
				|| this.checkLayeredTilesAttribute_YED(x, y, 
					function(tileProp) {
						if (tileProp["arrowImpassableLeft"] === "true") return true;
						if (tileProp["arrowImpassableLeft"] === "false") return false;
						if (tileProp["collision"] === "true") return true;
						if (tileProp["collision"] === "false") return false;
						return null;
					})) {
                return false;
            }
        ...
        if (d === 8) {
            if (!(arrows[index] & 2)
				|| this.checkLayeredTilesAttribute_YED(x, y, 
					function(tileProp) {
						if (tileProp["arrowImpassableUp"] === "true") return true;
						if (tileProp["arrowImpassableUp"] === "false") return false;
						if (tileProp["collision"] === "true") return true;
						if (tileProp["collision"] === "false") return false;
						return null;
					})) {
        ...
        if (d === 6) {
            if (!(arrows[index] & 4)
				|| this.checkLayeredTilesAttribute_YED(x, y, 
					function(tileProp) {
						if (tileProp["arrowImpassableRight"] === "true") return true;
						if (tileProp["arrowImpassableRight"] === "false") return false;
						if (tileProp["collision"] === "true") return true;
						if (tileProp["collision"] === "false") return false;
						return null;
					})) {
        ...
        if (d === 2) {
            if (!(arrows[index] & 8)
				|| this.checkLayeredTilesAttribute_YED(x, y, 
					function(tileProp) {
						if (tileProp["arrowImpassableDown"] === "true") return true;
						if (tileProp["arrowImpassableDown"] === "false") return false;
						if (tileProp["collision"] === "true") return true;
						if (tileProp["collision"] === "false") return false;
						return null;
					})) {
	...
	Game_Map.prototype.checkLayeredTilesAttribute_YED = function(x, y, matchCallback) {
		var layers = YED.Tilemap.Core.singleton.layers;
		// start at the top (layers on top trump layers on bottom)
		for (var i = layers.length - 1; i >= 0; i--) {
			var layer = layers[i];
			if (!layer.visible) continue;
			var tileId = layer._readMapTilesData(x, y);
			if (tileId <= 0) continue;
			var tileset = layer._getTileset(tileId);
			if (!tileset) continue;
			//var tile = tileset.getTile(tileId);
			//if (!tile) continue;
			//if (tile.terrain) continue;
			//if (tile.animation) tile = tile.animation[0].tileid; // Use the attributes of the first frame of an animated tile
			//var tileProp = tileset.data.tileproperties[tile];
			var localTileId = tileId - tileset.data.firstgid;
			var tileProp = tileset.data.tileproperties[localTileId];
			if (tileProp) {
				var match = matchCallback(tileProp);
				if (match == null) continue;
				return match;
			}
			// otherwise this layer does not have an opinion
		}
		return false;
	}
	
	Game_Map.prototype.checkLayeredTilesAttribute = function(x, y, attr) {
		if (!!YED.Tilemap.Core.noMap) return _Game_Map_checkLayeredTilesAttribute.call(this, x, y, attr);

		return this.checkLayeredTilesAttribute_YED(x, y, 
			function(tileProp) {
				if (tileProp[attr] === "true") return true;
				if (tileProp[attr] === "false") return false;
				return null;
			}
		);
	};
	...
	Game_Map.prototype.isLadder = function(x, y) {
		if (!!YED.Tilemap.Core.noMap) return _Game_Map_isLadder.call(this, x, y);
		return this.isValid(x, y) && this.checkLayeredTilesAttribute(x, y, "ladder");
	};

	Game_Map.prototype.isBush = function(x, y) {
		if (!!YED.Tilemap.Core.noMap) return _Game_Map_isBush.call(this, x, y);
		return this.isValid(x, y) && this.checkLayeredTilesAttribute(x, y, "bush");
	};

	Game_Map.prototype.isCounter = function(x, y) {
		if (!!YED.Tilemap.Core.noMap) return _Game_Map_isCounter.call(this, x, y);
		return this.isValid(x, y) && this.checkLayeredTilesAttribute(x, y, "counter");
	};

	Game_Map.prototype.isDamageFloor = function(x, y) {
		if (!!YED.Tilemap.Core.noMap) return _Game_Map_isDamageFloor.call(this, x, y);
		return this.isValid(x, y) && this.checkLayeredTilesAttribute(x, y, "damage");
	};

	Game_Map.prototype.terrainTag = function(x, y) {
		if (!!YED.Tilemap.Core.noMap) return _Game_Map_terrainTag.call(this, x, y);
		...
		if (this.isValid(x, y)) {
			...
			for (var i = layers.length - 1; i >= 0; i--) {
				...
				var tile = tileset.getTile(tileId);
				if (!tile) continue;
				if (!!tile["terrainTag"]) return tile["terrainTag"];
				// otherwise this layer does not have an opinion
			}
		}
		return 0;
    ...
    Game_Map.prototype.regionId = function(x, y) {
		if (!!YED.Tilemap.Core.noMap) return _Game_Map_regionId.call(this, x, y);

        var regions = this._yedTilemapData().region,
            index = this.width() * y + x;

		var toReturn = regions[index];
		if (toReturn === 0) toReturn = this.checkLayeredTilesAttribute(x, y, "region");
        return toReturn;
    };

    Game_CharacterBase.prototype.distancePerFrame = function() {
		if (!!YED.Tilemap.Core.noMap) return _Game_CharacterBase_distancePerFrame.call(this);
    ...
    Game_Event.prototype.setupPage = function() {
 		if (!YED.Tilemap.Core.noMap) this.setupInitPosition();

        _Game_Event_setupPage.call(this);
    };

    Game_Event.prototype.setupInitPosition = function() {
		if (!!YED.Tilemap.Core.noMap) return _Game_Event_setupInitPosition.call(this);
    ...
    Spriteset_Map.prototype.createTilemap = function() {
		if (!!YED.Tilemap.Core.noMap) return _Spriteset_Map_createTilemap.call(this);
    ...
    Tilemap.prototype.refresh = function() {
		if (!!YED.Tilemap.Core.noMap) return _Tilemap_refresh.call(this);
    ...
    Tilemap.prototype._createLayers = function() {
		if (!!YED.Tilemap.Core.noMap) return _Tilemap_createLayers.call(this);
    ...
    Tilemap.prototype._updateLayerPositions = function(startX, startY) {
		if (!!YED.Tilemap.Core.noMap) return _Tilemap_updateLayerPositions.call(this);
    ...
    Tilemap.prototype.updateTransform = function() {
		if (!!YED.Tilemap.Core.noMap) return _Tilemap_updateTransform.call(this);
    ...
    Tilemap.prototype._paintAllTiles = function(startX, startY) {
		if (!!YED.Tilemap.Core.noMap) return _Tilemap_paintAllTiles.call(this, startX, startY);
